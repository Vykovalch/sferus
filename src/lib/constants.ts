...
/**
 * Высота шапки в пикселях — держать синхронно с `h-16 lg:h-[72px]`
 * в `Header.tsx`: 64px на узких экранах, 72px от `lg`.
 *
 * Используется как компенсация `rootMargin` в IntersectionObserver, который
 * следит за формой поиска Hero: без неё триггер срабатывал бы по геометрии
 * вьюпорта, а не по факту перекрытия sticky-шапкой. От результата зависит
 * и поле поиска в шапке (от `xl`), и лупа на узких экранах.
 *
 * Высота нужна своя для каждой ширины (2026-09-19). Раньше на всех ширинах
 * бралось 72px, и на телефоне форма Hero считалась скрытой, пока её последние
 * ~8px ещё были видны под шапкой: лупа появлялась над выглядывающей карточкой.
 */
export const HEADER_HEIGHT_PX = 72;
export const HEADER_HEIGHT_MOBILE_PX = 64;
/** Ширина, с которой шапка становится 72px, — брейкпоинт `lg` Tailwind. */
export const HEADER_DESKTOP_MIN_WIDTH_PX = 1024;
