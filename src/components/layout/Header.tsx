...
export function Header({ session, cities }: HeaderProps) {
  ...
  return (
    <header className="sticky top-0 z-50 w-full border-b border-border bg-hero-bg/95 backdrop-blur-md">
      ...
      <PageContainer>
        {/* Разделы прижаты к логотипу, а свободное место копится в середине,
            где живёт поиск (решение владельца, 2026-09-23). Раньше строка
            распределяла элементы «поровну между краями», и в дележе участвовал
            схлопнутый блок поиска: на главной до прокрутки разделы висели
            в случайной точке (замер по снимку владельца — 180px от логотипа
            и 366px до «Разместить»), а при появлении поля подъезжали к логотипу.
            Теперь правая группа прижата к краю (`ml-auto`), поле забирает
            остаток (`flex-1`), и позиция разделов не зависит ни от прокрутки,
            ни от состава шапки.
            ...
            Правило: внутри группы отступ меньше, чем между группами. Поэтому
            у пунктов навигации 24px, у правой группы 12px, а между зонами —
            больше. Лупа живёт внутри правой группы, и её 12px до «Разместить»
        ...
        <div className="flex h-16 lg:h-[72px] items-center">
          <div className="flex w-full items-center gap-4 md:gap-8">
            ...
            {/* Правая часть сгруппирована по типу (решения владельца,
                ...
                Шаги внутри правой части: 8px на телефоне и 16px от 768px,
                внутри личного блока всегда 8px (решения владельца, 2026-09-23).

                На телефоне в правой части всего три иконки — лупа, колокольчик
                и аватар, — и делить их на группы нечем: черты там нет. При шаге
                16px лупа читалась как оторванная от ряда, поэтому на телефоне
                шаг общий и ряд ровный. От 768px в правой части появляются
                разнородные части (поле поиска рядом, дальше «Разместить»
                и аккаунт), и там разница шагов снова нужна. */}
            <div className="ml-auto flex items-center gap-2 md:gap-4">
              ...
              {/* Перед аватаром на 4px больше, чем между остальными иконками
                  (замер владельца на 408px, 2026-09-23): по центрам кнопок всё
                  ровно, 48px, но у лупы и колокольчика значок занимает 17–18px
                  внутри кнопки 40px, а кружок аватара залит целиком на 32px.
                  Щели между видимым были 33px и 24px, стали 33px и 28px.
                  Правило то же, что у поля поиска: выравниваем по значку,
              ...
              {session ? (
                <div className="flex items-center gap-2 [&>:last-child]:ml-1">
                  <NotificationsMenu />
