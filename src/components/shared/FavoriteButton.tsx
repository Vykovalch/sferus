 ...
 * в сохранённом.
 *
 * **Отметка теперь лежит прямо на фотографии, и на части снимков её не видно.**
 * Норма для значимой графики 3:1, замеры по цвету под иконкой:
 *
 * | фон под сердечком | несохранённое | сохранённое |
 * |---|---|---|
 * | чёрный | 3.5:1 | 4.1:1 |
 * | тёмно-серый `#2A2A2A` | 2.4:1 | 2.8:1 |
 * | средне-тёмный `#4A4A4A` | **1.5:1** | **1.8:1** |
 * | средний `#808080` | **1.5:1** | **1.3:1** |
 * | светлый `#E8E8E8` | 4.9:1 | 4.1:1 |
 * | белый | 6.1:1 | 5.1:1 |
 *
 * Провал приходится на середину шкалы — а это самый обычный фон фотографии.
 * В кружке на 80% было 3.8:1 … 6.1:1 и 3.2:1 … 5.1:1, то есть норма держалась
 * везде. Это осознанный размен владельца на чистый вид; вернуть читаемость,
 * не возвращая кружок, можно мягкой тенью на самой иконке — одна строка.
 */
const OVERLAY_CHIP =
