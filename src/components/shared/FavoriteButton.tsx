 ...
 *
 * Кружок 36px в 8px от верхнего и правого края снимка. Заливка белая на 80%
 ...
 *
 * Замеры при 80%, норма для значимой графики 3:1. Худший случай — чёрный
 * снимок, лучший — белый:
 * - несохранённое `--secondary` `#5B6468`: 3.8:1 … 6.1:1;
 * - сохранённое `--brand` `#8A6A00`: 3.2:1 … 5.1:1.
 *
 * **78% — предел, ниже опускать нельзя:** там сохранённое сердечко даёт ровно
 * 3.00:1 на чёрном снимке. Владелец просил прозрачнее дважды — с 90% до 80%
 * запас ещё остался, дальше его нет. Именно это ограничение в своё время
 * заставляло держать толстую обводку в цвет сердечка; на 80% она не нужна,
 * и обводка остаётся тем, чем должна быть, — почти невидимым краем.
 */
const OVERLAY_CHIP =
  "flex size-9 items-center justify-center rounded-full border border-white/70 bg-white/80 backdrop-blur-[2px] shadow-[0_1px_3px_rgba(0,0,0,0.1)] transition-colors hover:bg-white focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-secondary";
